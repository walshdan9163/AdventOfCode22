...
def solve(input_file):
    ...
    for pair in id_pairs:
        ...
        if (elf_one_ids[0] <= elf_two_ids[0] and elf_two_ids[0] <= elf_one_ids[1]):
            overlap_count += 1
        elif (elf_one_ids[0] <= elf_two_ids[1] and elf_two_ids[1] <= elf_one_ids[1]):
            overlap_count += 1
        elif (elf_two_ids[0] <= elf_one_ids[0] and elf_one_ids[0] <= elf_two_ids[1]):
            overlap_count += 1
        elif (elf_two_ids[0] <= elf_one_ids[1] and elf_one_ids[1] <= elf_two_ids[1]):
            overlap_count += 1
